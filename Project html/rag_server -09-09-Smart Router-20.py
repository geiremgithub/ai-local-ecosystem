...
SKRIPT_VERSJON = "v55 - debounce, topp 7 retrieval, topp 3 Qwen, kilde-ID"
...
print(
    f"[ChromaDB] Antall dokumenter i databasen: "
    f"{db_samling.count()}"
 )   
...
SIST_ENDRET = {}
DEBOUNCE_SEKUNDER = 1.0

def debounce_oppdatering(filsti):
    SIST_ENDRET[filsti] = time.time()
    def forsinket():
        time.sleep(DEBOUNCE_SEKUNDER)
        if time.time() - SIST_ENDRET[filsti] >= DEBOUNCE_SEKUNDER:
            oppdater_fil_i_database(filsti)
    Thread(target=forsinket, daemon=True).start()
...
class DokumentLytter(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if not event.is_directory:
            ...
            debounce_oppdatering(event.src_path)
...
def besvar_sporsmal(
    ...
    qwen_kontekster = dokument_kontekster[:TOP_K_QWEN]
    qwen_metadata = metadata_liste[:TOP_K_QWEN]



    kombinert_kontekst = "\n\n".join(
        f"--- Kilde: {meta.get('kilde')} (Chunk: {meta.get('chunk_nr')}) ---\n\n"
        f"{tekst}"
        for tekst, meta in zip(qwen_kontekster, qwen_metadata)
    )
    ...
    meldinger = [

        {
            ...
            "content": (
                ...
                "13. Du har ikke lov til å kombinere datoer, "
                "klokkeslett, tall eller navn fra forskjellige "
                "kilder. Hvis informasjonen ikke finnes samlet "
                "i én kilde, si at informasjonen ikke finnes "
                "samlet i kildene.\n"

                "14. Svar ALLTID i nøyaktig dette "
                "formatet med fire linjer:\n"
                ...
                "med 'Ja' eller 'Nei'>.\n"
                "KILDE: <nummeret på kilden som underbygger "
                "svaret, for eksempel 1>\n\n"
                "15. Ikke tell opp eller list opp filer (som fil 1, 2, 3 osv.) basert på antall tekstbiter (chunks). "
                "Hvis en fil nevnes, oppgi kun nøyaktig filnavn og filbane slik det står i metadataene for kilden."
    ...
    # TOKEN-STATISTIKK
    # ========================================================
    # Beregner antall genererte tokens i svaret og
    # genereringshastighet i tokens per sekund.
    #
    # Merk:
    # Dette er basert på den genererte teksten og er
    # ikke modellens totale tokenforbruk.
    # ========================================================
    ...
    print(
        f"[STATISTIKK] Genereringshastighet: "
    ...
    for linje in svar_linjer:
        ...
        elif linje.upper().startswith("KILDE:"):
            kilde_id = linje.split(":", 1)[1].strip()
    ...
    # ========================================================
    # BRUK KILDE-ID DIREKTE
    # ========================================================

    kilde_fil = ""
    chunk_nr = None

    if kilde_id and kilde_id.isdigit():

        indeks = int(kilde_id) - 1

        if indeks < len(qwen_metadata):

            kilde_fil = qwen_metadata[indeks]["kilde"]
            chunk_nr = qwen_metadata[indeks]["chunk_nr"]
...
def api_sporsmal():
    ...
    try:
        # 1. Sjekk om spørsmålet treffer en lokal handling (f.eks. mappeopprettelse)
        smart_melding = smart_router_handling(bruker_sporsmal)
        
        # 2. Hvis en lokal handling ble utført: Returner med en gang (Early Return)
        if smart_melding:
            return jsonify(
                {
                    "svar": smart_melding,
        ...
        # 3. Hvis IKKE en lokal handling: Gå veien om RAG og Qwen
        resultat = besvar_sporsmal(
            bruker_sporsmal,
            gjeldende_modus,
            temperatur
        )
        ...
        return jsonify(
            {
                "feil": f"Noe gikk galt på serveren: {feil}"
            }
        ), 500
