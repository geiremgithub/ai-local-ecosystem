...
SKRIPT_VERSJON = "v59-full-rag-logg-alltid-synlig-7b"
...
def _unik_kilde_navn(full_sti):
    """
    Lager en unik identifikator for en fil basert på hvilken overvåket mappe
    (MAPPESTIER) den ligger under, i tillegg til filnavnet. Dette hindrer at to
    filer med samme navn i ulike mapper (f.eks. "dokumenter/diverse.txt" og
    "sensitiv/diverse.txt") kolliderer og overskriver hverandre i databasen.
    """
    ...
    # Fallback dersom filen ikke ligger under noen kjent mappe
...
print("[System] Laster inn Qwen2.5-7B-Instruct...")
ai_modell = pipeline(
    "text-generation",
...
def besvar_sporsmal(bruker_sporsmal):
    ...
    for doc, meta in zip(alle_docs['documents'], alle_docs['metadatas']):
        if any(oi in doc.lower() for oi in ord_i_sporsmal):
            ...
            funnede_filer_hybrid.add(kilde_navn) 
    ...
    if ord_i_sporsmal:
        ...
        print("\n--- [SVAR: EKSAKT ORDSØK] ---")
        if funnede_lokasjoner:
            print(f"Ordet '{', '.join(ord_i_sporsmal)}' ble funnet i følgende filer og lokasjoner:")
            for kilde, chunk_nr in sorted(funnede_lokasjoner):
                print(f"  - {kilde} (Del {chunk_nr})")
        else:
            print(f"Ordet '{', '.join(ord_i_sporsmal)}' ble ikke funnet i noen av kildefilene.")
        print()
        return
